...
echo '<?php=$asignatura[permite_aplazado]?>';
session_start();
                                        ...
                                        <?php require_once('../assets/constants/check-reply.php') ;?>
                                          <div class="table-responsive">
                                            ...
                                            <table class="table table-striped table-bordered">
                                               ...
                                               <tbody>
                                                    <?php 
$stmt= $conn->prepare("SELECT RA.idresumen as IDRA ,AL.idalumno AS IDAL ,AL.apellidos AS Surname, AL.nombres AS Name, RA.prom1 AS P1, RA.prom2 AS P2, RA.prom3 AS P3, RA.prom4 AS P4, RA.promfinal AS PRF, RA.aplazado AS APLZ, RA.promdoc AS PD, RA.promjur AS PJ, RA.promdj AS PDJ, RA.cred AS CRD, RA.ptj AS PTJ, SA.descripcion as DESA, RA.id_criterio as CTR, RA.observacion AS OBS ,SA.permite_aplazado AS PA, SA.permite_jurado AS PJU from resumen_asignatura RA INNER JOIN alumno AL ON RA.id_alumno = AL.idalumno INNER JOIN subareas SA ON RA.id_subarea = SA.idsubarea where SA.idsubarea = '".$_GET['id']."'");
$stmt->execute();
...
//$result = $stmt->fetch(PDO::FETCH_ASSOC);
                                                   ...
                                                   <form method="post">
                                                      <tr>
                                                        ...
                                                        <td>
                                                            <input class="font box" value="<?=$value['P1']?>" placeholder="0" name="prom1">
                                                        </td>
                                                        <td>
                                                          <input  class="font box" value="<?=$value['P2']?>" placeholder="0" name="prom2" hidden="true">
                                                        <input  class="font box" value="<?=$value['P2']?>" placeholder="0" name="prom2" <?php if($value['P1'] == null || $value['P1'] == 0 ){ echo "disabled"; } else {echo "enable"; }?>>
                                                        </td>
                                                        <td>
                                                          <input class="font box" value="<?=$value['P3']?>" placeholder="0" name="prom3" hidden="true">
                                                          <input class="font box" value="<?=$value['P3']?>" placeholder="0" name="prom3" <?php if($value['P2'] == null || $value['P2'] == 0 ){ echo "disabled"; } else {echo "enable"; }?>>
                                                        </td>
                                                        <td>
                                                          <input class="font box" value="<?=$value['P4']?>" placeholder="0" name="prom4" hidden="true">
                                                          <input class="font box" value="<?=$value['P4']?>" placeholder="0" name="prom4" <?php if($value['P3'] == null ||  $value['P3'] == 0 ){ echo "disabled"; } else {echo "enable"; }?>>
                                                        </td>
                                                        <td>
